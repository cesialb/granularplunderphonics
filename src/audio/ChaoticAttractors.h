...
/**
 * @brief Factory class for creating attractor instances
 */
class AttractorFactory {
public:
    static std::unique_ptr<ChaoticAttractor> createAttractor(
        const std::string& type, double sampleRate)
    {
        if (type == "torus") {
    ...
    static std::unique_ptr<CustomAttractor> createCustomAttractor(
        size_t dimension, double sampleRate)
    {
        return std::make_unique<CustomAttractor>(dimension, sampleRate);
    }
};
